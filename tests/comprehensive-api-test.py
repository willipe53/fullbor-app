...
#
#   # Verbose mode (shows full request/response details)
...
#   ./comprehensive-api-test.py client-groups-script.yaml --username user@example.com \
#     --password mypassword
...
def _deep_format(value: Any, vars: T_VAR) -> Any:
    ...
    if isinstance(value, str):
        def repl(m):
            ...
            # Handle simple arithmetic expressions like "var + 1" or "var - 1"
            if ' + ' in expr:
                parts = expr.split(' + ', 1)
                if len(parts) == 2:
                    left, right = parts
                    left_val = _resolve_template_value(left.strip(), vars)
                    right_val = _resolve_template_value(right.strip(), vars)
                ...
                return str(_resolve_template_value(expr, vars))

        return re.sub(r"\{\{\s*([^}]+)\s*\}\}", repl, value)
    return value


def _resolve_template_var(key: str, vars: T_VAR) -> Any:
    """Resolve a template variable with security validation."""
...
def _evaluate_template_expression(expr: str, vars: T_VAR) -> Any:
    """Evaluate a template expression like 'var + 1' or 'var - 1'."""
    expr = expr.strip()
    ...
    if ' + ' in expr:
        ...
        # Simple variable reference
        return _resolve_template_value(expr, vars)


def _resolve_template_value(value: str, vars: T_VAR) -> Any:
    """Resolve a template value, which could be a variable or a literal."""
    value = value.strip()
    ...
    if _validate_template_key(value):
        if value in vars:
            return vars[value]

    # If not a number and not a valid variable, raise error
    raise ValueError(
        f"Invalid template value: '{value}' (not a number or valid variable)")
...
def _assert_json_checks(body: Any, checks: List[Dict[str, Any]], vars: T_VAR) -> None:
    ...
    for i, chk in enumerate(checks, 1):
        ...
        if "equals" in chk:
            exp = chk["equals"]
            # Handle template expressions in expected values
            if isinstance(exp, str):
                # Apply template formatting to handle {{var}} substitutions
                try:
                    exp = _deep_format(exp, vars)
                except Exception as e:
                    raise AssertionError(
                        f"[{path}] failed to format template '{exp}': {e}")
            # Handle type conversion for numeric comparisons
            if isinstance(exp, str) and exp.isdigit() and isinstance(actual, int):
                exp = int(exp)
            elif isinstance(exp, int) and isinstance(actual, str) and actual.isdigit():
                actual = int(actual)
...
def run_test(client: httpx.Client, tc: TestCase, vars: T_VAR, verbose_level: int = 0) -> TestResult:
    ...
    try:
        ...
        # Show detailed request information if verbose level 1 or higher
        if verbose_level >= 1:
            print(f"\n🔍 REQUEST DETAILS:")
            ...
            if data_json is not None:
                print(f"   Payload: {json.dumps(data_json, indent=2)}")
        ...
        if tc.expect_json:
            _assert_json_checks(body, tc.expect_json, vars)
...
def main():
    ap = argparse.ArgumentParser(
        description="Enhanced YAML-driven API test runner (run from tests/ directory)")
    ap.add_argument(
        "plan", help="YAML test file (e.g., client-groups-script.yaml)")
    ap.add_argument("--base-url", default="https://api.fullbor.ai/v2",
                    help="Base URL to prepend to relative paths (default: https://api.fullbor.ai/v2)")
    ap.add_argument(
    ...
    ap.add_argument("--verbose", "-v", action="count", default=0,
                    help="Show detailed output: 1=test progress + full request/response details")
    ap.add_argument("--quiet", "-q", action="store_true",
                    help="Suppress normal output (only show failures and summary)")
    ap.add_argument("--parallel", type=int, metavar="N",
                    help="Run tests in parallel with N workers (default: sequential)")
    ap.add_argument("--no-export", action="store_true",
                    help="Disable automatic export to timestamped JSON file")
    ap.add_argument("--no-color", action="store_true",
                    help="Disable colored output")

    # Authentication options (always enabled)
    ...
    # Set up output teeing to both console and file (unless disabled)
    output_file = None
    if not args.no_export:
        timestamp = datetime.now().strftime("%Y%m%dT%H%M%S")
        output_file = f"{timestamp}testoutput.txt"
        ...
        with open(output_file, 'w') as f:
            ...
            try:
                ...
                run_tests_with_args(args, timestamp)
            finally:
        ...
        # Run without teeing
        run_tests_with_args(args, None)


def run_tests_with_args(args, timestamp):
    """Run the actual tests with the given arguments."""
    ...
    # Add timestamp variable for unique test data
    vars['timestamp'] = datetime.now().strftime("%Y%m%d%H%M%S")
    if args.vars:
        try:
    ...
    def run_single_test(tc: TestCase) -> TestResult:
        ...
        return run_test(client, tc, vars, args.verbose)

    if max_workers is None:
        # Sequential execution with progress tracking
        if RICH_AVAILABLE and not args.no_color and args.quiet:
            # Use progress bar only in quiet mode
            with Progress(
                SpinnerColumn(),
            ...
            # Default execution with test progress (current verbose behavior is now default)
    ...
    # Export results as JSON (for structured data) if teeing is enabled
    if timestamp is not None:
        try:
            json_file = f"{timestamp}testoutput.json"
            _export_results(results, "json", json_file)

            if not args.quiet:
                print(f"\n📄 Results exported to: {json_file}")
                print(f"📄 All output logged to: {timestamp}testoutput.txt")

        except Exception as e:
            print(f"Failed to export results: {e}", file=sys.stderr)
            # Don't exit on export failure, just warn
